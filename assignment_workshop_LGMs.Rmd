...
Latent variables are unobserved variables. The parameter of the unobserved trajectories such as intercept and slope are called latent variables.
...
path <- here::here() # this take the path that your R session is currently saving files to
#If you want another path you can also setup the path manually with: path<-"C:/Users/Documents"

setwd(path)
wisc <- read.csv("~/LGC_Workshop/wisc.csv")[,-1]
...
In the previous module, we modeled a linear growth model. Yet, it is also possible to model non-linear growth in lavaan like a quadratic trajectory. For this you need to add a third parameter called q for quadratic that will get the same loadings as for the slope but squared. 
...
**Assignment 18: You have an error message. What does it mean?**

**Assignment 19: Perform a statistical test to compare model fit. Does the linear or non-linear model fit better?**

```{r class.source = "fold-show", comparefit2, message=FALSE,warning=FALSE,include=TRUE}
# Compare model fit
anova(fit_quad_growth_model,fit_linear_growth_model)
...
**Assignment 20: Create a non-linear (basis) growth model, fit it to the verbal data, and output results. Then compare the model fit with the previous models.**
...
# Compare model fit linear and baseline
anova(fit_basis_growth_model,fit_linear_growth_model)
#Here it looks like the basis model fits better

# Compare model fit quadratic and baseline
anova(fit_basis_growth_model,fit_quad_growth_model)
#Here it looks like there is little difference between those two models. 
```
...
**Assignment 21: Add educational level of the mother (moedu in the data file) as predictor of baseline scores and change in the non-linear model.**
...
**Assignment 23: Perform the same steps as in assignments 20 and 21 but now for processing speed at baseline. Does processing speed relate to verbal baseline scores? And to change?**
...
**Assignment 24: Perform the same steps as in assignments 7, 8 and 14 but take into account the uneven time intervals in your loadings. Compare the three fit of the three models. do you have the same result as in the previous assignments?**
...
# Compare linear model fit between the model not taking into account the uneven intervals and the one twking into account the uneven intervals
...
### Time-invariant or time-varying predictors of change
...
For instance, in assignment 20, level of education of the mother is a time-invariant predictors. 
...
**Assignment 25: Use processing speed as a time-varying predictors of the verbal measurement at each time points. How are the intercept and slope of the verbal measures? Does processing speed predict verbal measures the same way across time points?**
...
It seems like processing speed predicts verbal measurements at each time points. We can find if the changes covary and how they are influenced by each other. 

**Assignment 26: Create two non-linear (basis) growth models, one for verbal and one for processing speed. In a first model, try to correlate the changes of the two metrics. In a second model, explore how baseline level of one metric predict the changes in the other.  **
...
Data simulation is an useful tool to test your models and do power analysis, 

**Assignment 27: Simulate a linear growth model with 4 waves. The intercepts should have a mean of 100 and a variance of 15. The slope should have a mean of 5 and a variance of 10. The intercept-slope covariance should have a mean of 3.**
...
**Assignment 28: Simulate a linear growth model with three groups.**
...
**Assignment 29: Use the previous simulate data model to test the effect size of a predictor on the slope. **
