...
Latent variables are unobserved variables. The parameters of the unobserved trajectories such as intercept and slope are called latent variables.

<!-- Maybe the last sentence would be more clear if it was phrased like "In latent growth models the growth trajectories are represented by the intercept and slope parameters, which are considered latent variables" (depends on how familiar with LGM they already are, but it confused me in the beginning)-->
...
<!-- Time-varying predictors are variables that can change across different time points and are used to predict repeated measures/outcome variable at those corresponding time points. They explain within-person variation over time, as each predictor is linked to the outcome at a specific time point. In our example for instance, to have a time-varying predictor we would need measurements at T6, T7, T9 and T11 
-->


<font size="4">**Assignment 25: Use processing speed as a time-varying predictors of the verbal measurement at each time points. How are the intercept and slope of the verbal measures? Does processing speed predict verbal measures the same way across time points?**</font>
...
<font size="4">**Assignment 26: Perform the same steps as in assignments 7, 8 and 14 but take into account the uneven time intervals in your loadings. Compare the three fit of the three models. do you have the same result as in the previous assignments?**</font>
...
<font size="4">**Assignment 28: Within that model, explore how baseline level of one metric predict the changes in the other. How do the predict each other?**</font>
